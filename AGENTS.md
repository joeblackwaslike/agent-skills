...
**Local auto-update:** a `SessionStart` hook in `~/.claude/settings.json` runs `claude plugin update agent-skills@agent-marketplace` once per week (on first session start after the Monday workflow commits). To opt out, set `AGENT_SKILLS_AUTO_UPDATE=false` in your environment or under `env` in `~/.claude/settings.json`.
...
**Senior-engineer judgment for any development session:** `senior-engineering-best-practices` for career-built instincts most agents lack by default — verifying the current authoritative source before proposing a workaround or answering from memory, progressive root-cause discipline (don't stop at the first plausible cause; an old "verified" claim blocking a fix is a hypothesis to re-test, not settled fact), extracting and propagating a real fix to every consumer with the same shape, blast-radius/reversibility judgment, prior-art and code-smell checks, dependency judgment, observability, and performance pattern-matching. Routes to `test-driven-development`/`solid-principles`/`domain-driven-design`/`superpowers` skills for situations they already cover.
...
**Resuming or auditing prior work:** `recovering-session-context` reconstructs what happened in earlier sessions from their transcripts and surfaces what was never resolved. Two modes — resume (the last session, then continue) and audit (a lookback window, then report). Backs the `/pickup` and `/outstanding` commands, and is worth invoking directly before declaring a thread finished.

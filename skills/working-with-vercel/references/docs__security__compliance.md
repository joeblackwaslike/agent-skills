...
Customers subject to HIPAA may enable [Vercel Secure Compute (available on Enterprise plans)](/docs/networking/secure-compute) for additional layers of protection. This allows customers to have more control over which resources they allow to have access to their information through:

- Private, isolated cloud environments
- Dedicated outgoing IP addresses

[VPC peering and VPN support](/docs/networking/secure-compute#vpn-support) (built on top of Secure Compute) allows customers to create fewer entry points into their networks by establishing secure tunnels within their AWS infrastructure.
...
> Compute](/docs/networking/secure-compute) to create a private, isolated cloud environment

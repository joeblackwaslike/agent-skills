...
For most domains purchased through or transferred into Vercel, you can manage WHOIS privacy from the **Registrant Information** section in the domain settings. WHOIS privacy availability depends on the domain's top-level domain (TLD). See [Supported Domains](/docs/domains/supported-domains) to check whether a TLD supports WHOIS privacy. If a domain supports WHOIS privacy, it is enabled by default when purchased through Vercel.

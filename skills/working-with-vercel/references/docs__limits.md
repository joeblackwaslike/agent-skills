...
See the [Limits and Pricing](/docs/speed-insights/limits-and-pricing) doc for more details about the limits of the Speed Insights feature on Vercel.

## Cron Jobs

See the Cron Jobs [limits](/docs/cron-jobs/usage-and-pricing) section for more information about the limits of Cron Jobs on Vercel.
...
You are able to build `100` [Deployments](/docs/deployments) every `3600` seconds (1 hour). Should you hit the rate limit, you will need to wait another hour before you can build a deployment again. This corresponds to the `api-deployments-free-per-hour` limit in the table below.

...
sha256: "cdeaaf2410d74073901a6c38eaa1673ebd5c682d5b55e1f5e8cb051ad1491112"
---
...
- [Microfrontends Configuration](https://vercel.com/docs/microfrontends/configuration?from=related&source_path=%2Fdocs%2Fmicrofrontends%2Flocal-development&source_site=vercel-docs&relationship=related) — Configure your microfrontends.json.
...
- ### Run the microfrontends proxy
  ...
  To test several microfrontends together, start each one in its own repository, then pass all of them to a single proxy as a space-separated list:
  ```bash
  microfrontends proxy --local-apps web docs
  ```
  You run only one proxy regardless of how many applications you run locally. The proxy routes requests for the listed applications to your local processes and sends everything else to the `fallback` URLs in your configuration.

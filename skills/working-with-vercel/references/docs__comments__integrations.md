...
sha256: "156f55e8cb471c162ae8739b4ba17628a21f7cdd89964edb34d576c517d5d3af"
---
...
1. Go to [the Vercel app for Slack in the Vercel Integrations Marketplace](https://vercel.com/marketplace/slack)
...
If you configured the Vercel app before October 4th, 2023, the updated app requires new permissions. You must reconfigure the app to subscribe to new comment threads and link new channels.
...
When a new comment is created on a deployment, the Vercel app will create a matching thread in each of the subscribed Slack channels. The first post will include:
...
Use the following Slack command to list all available options:
...
To receive comment notifications as DMs from the Vercel app, you must link your Vercel account in Slack by entering the following command in any Slack channel, thread or DM:

```bash
/vercel login
```

### Vercel app for Slack command reference
...
| `/vercel whoami`                        | Check which account you're logged into the Vercel app with           |

...
- [Server-Timing response headers will pass through to the client](https://vercel.com/changelog/server-timing-header?from=related&source_path=%2Fdocs%2Fproject-configuration%2Fvercel-json&source_site=vercel-docs&relationship=related)
...
The `routes` property lets you define routing rules using [PCRE-compatible regular expressions](https://en.wikipedia.org/wiki/Perl_Compatible_Regular_Expressions). You can use `routes` alongside [`rewrites`](/docs/project-configuration/vercel-json#rewrites), [`redirects`](/docs/project-configuration/vercel-json#redirects), [`headers`](/docs/project-configuration/vercel-json#headers), [`cleanUrls`](/docs/project-configuration/vercel-json#cleanurls), and [`trailingSlash`](/docs/project-configuration/vercel-json#trailingslash).

For common use cases, use those higher-level properties instead. See [Routes vs higher-level properties](#routes-vs-higher-level-properties) for guidance on when to use each.
...
| `has`        |               | `Array`    | An array of [`has` objects](#conditional-matching-with-has-and-missing) with the `type`, `key`, and `value` properties. Used for conditional path matching based on the **presence** of specified properties.                                                                                              |
| `missing`    |               | `Array`    | An array of [`missing` objects](#conditional-matching-with-has-and-missing) with the `type`, `key`, and `value` properties. Used for conditional path matching based on the **absence** of specified properties.                                                                                           |
| `mitigate`   |               | `Object`   | An object with the property `action`, which can either be "challenge" or "deny". The specified action performs [mitigation](/docs/vercel-firewall/vercel-waf/custom-rules#custom-rule-configuration) on requests that match the route.                                                                     |
| `transforms` |               | `Array`    | An array of `transform` objects. Transform rules let you append, set, or remove request/response headers and query parameters at the edge. See [transform examples](#transform-examples).                                                                                                                  |
| `env`        |               | `String[]` | A whitelist of [environment variable](/docs/environment-variables) names whose values replace `$VAR` or `${VAR}` references in `dest` values at request time. Only variables listed here are available for expansion. See [using environment variables in routes](#using-environment-variables-in-routes). |
...
- `handle`: A special route type (e.g., `"handle": "filesystem"`) that controls routing phases. Use [`rewrites`](/docs/project-configuration/vercel-json#rewrites) instead, which checks the filesystem by default.
- `important`: A boolean that forces the route to take precedence. Use [`redirects`](/docs/project-configuration/vercel-json#redirects) or [`headers`](/docs/project-configuration/vercel-json#headers) instead.
- `override`: A boolean that overrides the matched path in the filesystem. Use [`rewrites`](/docs/project-configuration/vercel-json#rewrites) instead.
...
You can reference [environment variables](/docs/environment-variables) in route `dest` values and transform `args` using `$VAR` or `${VAR}` syntax. Add the variable names to the `env` array on the route or transform so they're available for expansion at request time. Values come from your project's environment variables.
...
You can combine transforms with the [`has` and `missing` properties](#conditional-matching-with-has-and-missing) and the matching conditions in the [Transform key object definition](#transform-key-object-definition).

### Routes vs higher-level properties

For common use cases like redirects, rewrites, and custom headers, the higher-level [`rewrites`](/docs/project-configuration/vercel-json#rewrites), [`redirects`](/docs/project-configuration/vercel-json#redirects), [`headers`](/docs/project-configuration/vercel-json#headers), [`cleanUrls`](/docs/project-configuration/vercel-json#cleanurls), and [`trailingSlash`](/docs/project-configuration/vercel-json#trailingslash) properties offer a more concise alternative to `routes`. You can use both in the same configuration.

The following examples show how common `routes` patterns map to higher-level properties.

#### Route parameters

With `routes`, you use a [PCRE-compatible regular expression](https://en.wikipedia.org/wiki/Perl_Compatible_Regular_Expressions) named group to match the ID and then pass that parameter in the query string. The following example matches a URL like `/product/532004` and proxies to `/api/product?id=532004`:
...
With [`rewrites`](/docs/project-configuration/vercel-json#rewrites), named parameters pass through in the query string. The following example is equivalent to the `routes` usage above, but uses `rewrites` instead:
...
With [`redirects`](/docs/project-configuration/vercel-json#redirects), you disable the `permanent` property to use a 307 Temporary Redirect. Also, `redirects` are always processed before `rewrites`. The following example is equivalent to the `routes` usage above, but uses `redirects` instead:
...
With [`headers`](/docs/project-configuration/vercel-json#headers), this is no longer necessary since that is the default behavior. The following example is equivalent to the `routes` usage above, but uses `headers` instead:
...
With `routes`, you need to escape a dot with two backslashes, otherwise it would match any character [PCRE-compatible regular expression](https://en.wikipedia.org/wiki/Perl_Compatible_Regular_Expressions). The following example matches the literal `atom.xml` and proxies to `/api/rss` to dynamically generate RSS:
...
With [`rewrites`](/docs/project-configuration/vercel-json#rewrites), the `.` is not a special character so it does not need to be escaped. The following example is equivalent to the `routes` usage above, but instead uses `rewrites`:
...
With `routes`, you use [PCRE-compatible regular expression](https://en.wikipedia.org/wiki/Perl_Compatible_Regular_Expressions) negative lookahead. The following example proxies all requests to the `/maintenance` page except for `/maintenance` itself to avoid an infinite loop:
...
With [`rewrites`](/docs/project-configuration/vercel-json#rewrites), the regex needs to be wrapped in a capture group. The following example is equivalent to the `routes` usage above, but instead uses `rewrites`:
...
With [`rewrites`](/docs/project-configuration/vercel-json#rewrites) / [`redirects`](/docs/project-configuration/vercel-json#redirects) / [`headers`](/docs/project-configuration/vercel-json#headers), the `source` property is case-sensitive so you don't accidentally create duplicate content.

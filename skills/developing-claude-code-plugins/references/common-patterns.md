...
**Real example:** `superpowers-developing-for-claude-code` — single skill with comprehensive documentation, scripts for self-updating, 40+ reference files, no MCP servers or hooks.
...
**Real example:** `superpowers-chrome` — MCP server provides browser control tools, skill teaches Claude how to use those tools effectively.

...
3. **Start Claude Code with your plugin loaded** (no install step needed):

   ```bash
   claude --plugin-dir ./my-plugin
   ```

   See `references/plugin-structure.md` for `--plugin-dir` details and the marketplace.json format (used for distribution, not local dev).
...
1. **Load for testing** (no install step):
...
3. **Iterate** — after changes run `/reload-plugins` to pick them up mid-session. For MCP/LSP server changes, a full restart is required.

### Phase 5: Debug and Refine

**Start here:** run `claude plugin validate` (or `/plugin validate` in-session) to catch structural errors immediately.

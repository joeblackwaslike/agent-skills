...
- **No API balance funding the hosted bots? Run them locally instead of waiting.** `ai-review watch --pr <n> &` (backgrounded, before or right after opening the PR) polls the PR and posts re-reviews on every push using the developer's own Claude Max/ChatGPT Pro subscription auth — under the *exact same* `anthropicreviewbot`/`codexreviewbot` identities the hosted path uses, so every mechanic in this runbook (thread enumeration, reaction rating, the `~7.5 minute` timing note above, dismiss-the-stuck-reviewer) applies unmodified. It's a local, personal-use substitute for the hosted webhook bots, not a different reviewer to account for — don't add it to the required-bot list, it *is* those bots. Exits on its own when the PR merges or closes; kill it manually if you abandon the PR first. See `src/watch.ts` in `ai-review-bot`.
...
**Validate before applying — this is non-negotiable.** Invoke `superpowers:receiving-code-review` and run its Read → Understand → Verify → Evaluate → Respond → Implement loop. Do not perform agreement and do not blindly apply suggestions, including AI suggestions and one-click ` ```suggestion ` blocks:
...
- **The guard must be restated explicitly when this loop is delegated to a subagent — it does not carry over on its own.** "Drive this PR to mergeable" is a goal, not a bound; a subagent following it round after round has no reason to treat round 6 differently from round 1 unless told to. State a concrete cap *in the delegation prompt itself* (e.g. "stop after 3 consecutive rounds where every finding is Low-severity or confirms a prior fix as correct — resolve what's real, decline the rest with cited evidence, and report back rather than continuing") and re-state it again in any follow-up message that resumes the subagent with a fresh round of findings. Observed on `ai-review-bot` PRs #67/#68 (see `docs/post-mortem-review-loop-churn.md` in that repo): two delegated subagents each independently recognized this section's own "diminishing returns" tells (below) partway through, named the pattern out loud, and still ran 3-5 more rounds anyway, because recognizing the guard is not the same as having a stated number to stop at.
- **A fast-polling local watcher (e.g. `ai-review watch --interval 45-60s`) attached to the PR interacts badly with this guard, not just with poll cadence.** A hosted webhook reviewer re-reviews once per push at human commit cadence; a 45-60s local watcher re-reviews every push at whatever cadence the loop itself is pushing — which, for an agent, can be faster than the reviewer's own turnaround. Every fix pushed to satisfy round N becomes fresh diff for round N+1 before round N's replies have even settled, and the max-iteration counter climbs on real work each time. Pause or kill the watcher before pushing a batch of review-response fixes, and only restart it for a single final confirmation pass — don't leave it running unattended through an active fix-and-repush cycle. **Prose telling the operator to pause the watcher is not sufficient on its own** — it was already documented before the `ai-review-bot` #67/#68 incident and was recognized-but-not-enforced mid-run. `ai-review-bot`'s `watch` command now backs it with a mechanical control, the model to follow for any similar local-loop tool: a conservative 300s (5 min) default poll interval (not just a suggestion — the flag can still override it, but the default is the safe value), plus a circuit breaker independent of interval — the loop stops itself once a single provider posts 3 reviews within a 15-minute rolling window, regardless of how fast it's being pushed into, with an explicit escape hatch (`--no-circuit-breaker`) for a deliberate, already-supervised run. Cap *posts*, not just polling cadence — a fast `--interval` alone doesn't cause harm until it actually results in repeated posts outrunning triage.
...
- **A serverless-backed reviewer can silently stall on a large PR past its platform's function timeout** — the failure mode looks like a bot that simply never ran: no review, no error, no signal anywhere. Observed on `ai-review-bot`: a deep-tier PR's agents ran 150–300s each; 8 sequential agents didn't fit under an 800s ceiling, the platform killed the function mid-run, and everything already produced (findings from agents that had completed) was lost with it, silently. The fix that actually holds is two independent, composable mechanisms, not a retry:
  - **Wall-clock budget**, checked before dispatching each unit of work. Reserve enough margin for both that unit's own worst-case duration *and* the tail work still owed after the last one (merging results, generating a summary, submitting) — a run that can't fit another unit in the remaining budget submits what it has, with an explicit "partial" notice, instead of starting work the platform will kill mid-flight.
  - **Bounded wait-with-a-ceiling**, for anything the loop is blocking on (a peer review, a slow check) — so it can never wait unboundedly.

  If a reviewer you're driving toward approval exhibits this — genuinely never posts, not even an error — look for whether it has either mechanism before concluding it's stuck-and-dismissable (see below): a bot that's infra-silent because it lacks a budget/ceiling is a different failure than one that's mechanically recycling stale findings, and re-triggering it manually is a workaround for the gap, not a fix for it.
...
**Dismiss mechanic and payload:** [`code-review-via-api.md` § Dismiss a review](./code-review-via-api.md#dismiss-a-review) — includes the stacked-reviews caveat (dismiss every `CHANGES_REQUESTED` from that reviewer, not just the latest, or an earlier one still counts toward `reviewDecision`).

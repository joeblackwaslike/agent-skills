...
- **A serverless-backed reviewer can silently stall on a large PR past its platform's function timeout** — the failure mode is a bot that appears to have simply never run: no review, no error, no signal anywhere. Observed on `ai-review-bot`: a deep-tier PR's agents ran 150–300s each, and 8 sequential agents didn't fit under an 800s ceiling — the platform killed the function mid-run and everything already produced (findings from agents that had completed) was lost with it, silently. The fix that actually holds is two independent, composable mechanisms, not a retry: (1) a wall-clock **budget** checked before dispatching each unit of work — reserving enough margin for both that unit's own worst-case duration *and* the tail work still owed after the last one (merging results, generating a summary, submitting) — so a run that can't fit another unit in the remaining budget submits what it has (with an explicit "partial" notice) instead of starting work the platform will kill mid-flight; (2) a bounded **wait-with-a-ceiling** for anything the loop is blocking on (a peer review, a slow check), so it can never wait unboundedly. If a reviewer you're driving toward approval exhibits this — genuinely never posts, not even an error — look for whether it has either mechanism before concluding it's stuck-and-dismissable (see below): a bot that's infra-silent because it lacks a budget/ceiling is a different failure than one that's mechanically recycling stale findings, and re-triggering it manually is a workaround for the gap, not a fix for it.
...
**Dismiss mechanic and payload:** [`code-review-via-api.md`](./code-review-via-api.md#6-reading-review-state) — per-review, keyed on the review `id`, and blocking reviews from one reviewer **stack**: dismiss every `CHANGES_REQUESTED` from that reviewer, not just the latest, or an earlier one still counts toward `reviewDecision`.

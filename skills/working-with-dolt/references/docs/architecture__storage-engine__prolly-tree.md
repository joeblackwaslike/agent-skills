...
3. **Hash each Chunk**: You now have the leaf nodes of the Prolly Tree. Compute the content address of each block by applying a strong hash function to its contents. You store the contents in [a content addressed block store](/architecture/storage-engine/block-store). Here our blocks have been addressed and stored in the block store:
...
Prolly Trees themselves are [block store](/architecture/storage-engine/block-store) agnostic. So if you only care about the Prolly Tree data structure, you can skip this section.

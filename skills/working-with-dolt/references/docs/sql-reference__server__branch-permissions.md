...
source: "https://www.dolthub.com/docs/sql-reference/server/branch-permissions.md"
fetched_at: "2026-06-15T20:08:28.186Z"
sha256: "dc85a2063399e72a3b1dc1c23649c0b31ab523dffd028c3c6fbccf652ebe7f48"
---
...
| permissions | SET("admin", "write", "merge", "read") | utf8mb4_0900_ai_ci |
...
| merge      | Grants the ability to merge into a branch with `dolt_merge`. Does not allow arbitrary writes.                                                                                                                                       |
...
The branch permissions system tables, `dolt_branch_control` and `dolt_branch_namespace_control`, are both built on top of [users and privileges](/sql-reference/server/access-management) system.
...
The rules for selecting a `branch_control.db` file [are the same as for the privileges file](/sql-reference/server/access-management).
...
Please refer to the [setup section](#setup) before continuing this example.
This example shows the `merge` permission in action.
We add the `merge` permission to the `testuser` user, which allows that user to modify the contents of our `feature` (default) branch, while not allowing any modifications to the `main` branch except through the `dolt_merge` and `dolt_commit` procedures.

```sql
$ mysql --user=root
mysql> USE example;
mysql> INSERT INTO dolt_branch_control VALUES ('%', '%', 'root', '%', 'admin');
mysql> INSERT INTO dolt_branch_control VALUES ('%', 'main', 'testuser', '%', 'merge');
mysql> CREATE TABLE test (pk BIGINT PRIMARY KEY);
mysql> exit;
...
mysql> CALL DOLT_CHECKOUT("-b", "feature");
mysql> INSERT INTO test VALUES (1), (2);
mysql> CALL DOLT_COMMIT("-Am", "Added rows");
mysql> CALL DOLT_CHECKOUT("main");
mysql> CALL DOLT_MERGE("feature");
